...
export default function StreetScreen({
  ...
  return (
    <Container>
      {currentStreet && <TaskList currentStreet={currentStreet} />}
...
function TaskList({ currentStreet }) {
  const { tasks, addTask, loading } = useTasks(currentStreet);
    ...
    <>
      <List>
        {tasks.length === 0 ? (
          <TaskListHeader>
            <h1>No Tasks</h1>
            <p>Click the button below to add a task to this Street</p>
          </TaskListHeader>
        ) : (
          tasks.map((task) => (
